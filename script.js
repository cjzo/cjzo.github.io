...
document.getElementById('addOrbiter').addEventListener('click', (e) => {
  e.preventDefault();
  const newOrbiter = document.createElement('div');
  newOrbiter.className = 'orbiter';
  blob.appendChild(newOrbiter);
});

document.getElementById('removeOrbiter').addEventListener('click', (e) => {
  e.preventDefault();
  if (blob.children.length > 1) {
      blob.removeChild(blob.lastChild);
  }
});
